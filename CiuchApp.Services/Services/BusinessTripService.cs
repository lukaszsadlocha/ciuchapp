...
namespace CiuchApp.Services
{
    public class BusinessTripService : BaseService<BusinessTrip>, ICrudService<BusinessTrip>
    {
        ...
        public async Task<BusinessTrip> GetByIdAsync(int id) => await GetBusinessTripsAndIncludedValues().FirstOrDefaultAsync(x => x.Id == id);

        public async Task<List<BusinessTrip>> GetAllAsync() => await GetBusinessTripsAndIncludedValues().ToListAsync();
